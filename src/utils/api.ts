...
export const fetchPosts = async (): Promise<Post[]> => {
  ...
  return json.data
    .filter((post: DBPost) => post.slug != null && post.slug !== "")
    .map((post: DBPost) => {
      // Extract just the last segment of the slug if it contains path separators
      // This handles cases like "blog/post/my-slug" -> "my-slug"
...
export const fetchPostBySlug = async (slug: string): Promise<DBPost | null> => {
  ...
  // Try with the clean slug first
  const queryString = new URLSearchParams({
    fields: "*,gallery.directus_files_id",
    filter: JSON.stringify({
      _or: [{ slug: { _eq: slug } }, { slug: { _eq: `blog/post/${slug}` } }],
    }),
  });
  const response = await fetch(
    "https://panel.braga.co.id/panel/items/posts?" + queryString,
...
export const fetchTestimonials = async (): Promise<Testimonial[]> => {
  ...
  return json.data.map((testimonial: DBTestimonial) => ({
    ...
    personImage: testimonial.person_image
      ? `https://panel.braga.co.id/panel/assets/${testimonial.person_image}`
      : null,
...
export const fetchTestimonialsByWorkId = async (
  workId: number,
): Promise<Testimonial[]> => {
  const accessToken = import.meta.env.DIRECTUS_ACCESS_TOKEN;
  const queryString = new URLSearchParams({
...
export const fetchLandingPage = async (): Promise<LandingPageData> => {
  ...
  return {
    hero: {
      ...
      partnerLogos: data.h_partnerLogos.map(
        (logo) =>
...
export const searchBlogContent = async (
  query: string,
): Promise<{ works: Work[]; posts: Post[] }> => {
  const accessToken = import.meta.env.DIRECTUS_ACCESS_TOKEN;
  ...
  const [worksResponse, postsResponse] = await Promise.all([
    fetch("https://panel.braga.co.id/panel/items/works?" + worksQueryString, {
      headers: { Authorization: "Bearer " + accessToken },
    }),
    fetch("https://panel.braga.co.id/panel/items/posts?" + postsQueryString, {
      headers: { Authorization: "Bearer " + accessToken },
    }),
  ]);
